...
public class TemplateUtil {
    ...
    public static void replaceTextPlaceholdersInElement(ContentAccessor element,
                                                        Map<String, String> placeholdersValues,
                                                        Boolean replaceEmptyWithBlank) {
        if (element == null) {
            log.warn("Element is null, cannot replace placeholders");
            return;
        }
        if (placeholdersValues == null || placeholdersValues.isEmpty()) {
            log.warn("No placeholders provided for replacement");
            return;
        }
        List<Text> placeholders = getTextsPlaceholdersFromContentAccessor(element);
        if (placeholders.isEmpty()) {
            log.warn("No placeholders found in element");
        }
        replaceValuesInTextPlaceholders(placeholders, placeholdersValues, replaceEmptyWithBlank);
    }
    ...
    public static P findParagraphWithPlaceholder(WordprocessingMLPackage template, String placeholder) {
        List<Object> paragraphs = getAllElementsFromObject(template.getMainDocumentPart(), P.class);
        for (Object obj : paragraphs) {
            P paragraph = (P) obj;
            List<Object> texts = getAllElementsFromObject(paragraph, org.docx4j.wml.Text.class);
